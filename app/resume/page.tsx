        ...
        <div className="space-y-10">
          {projects.map((item, index) => (
            <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="text-xs font-medium text-[#aa002a] tracking-wider uppercase md:pt-1">
                {item.tech}
              </div>
            ...
            >
              <div>
                <div className="flex justify-between items-start">
                  <h3 className="text-sm font-medium text-gray-900 group-hover:text-[#aa002a] transition-colors">
                    {cert.title}
                  </h3>
      ...
      {selectedImage && (
        ...
        >
          <div className="relative max-w-4xl w-full bg-white p-2 rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button 
              onClick={() => setSelectedImage(null)}
              className="absolute -top-10 right-0 text-white text-xl font-bold hover:text-[#aa002a] transition-colors"
            >
              ✕ Close
            </button>
